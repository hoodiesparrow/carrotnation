...
const SalesByPrice = defineAsyncComponent(() => import("@/components/charts/SalesByPrice.vue"));
...
export default {
  ...
  setup() {
    ...
    const cycle = ref("");
    cycle.value = `${String(date.getFullYear()).slice(2)}${String(date.getMonth() + 1).padStart(
      2,
      "0"
    ...
    store.dispatch("requestByPrice", bypriceQuery.value).then((res) => {
      for (let i = 0; i < 5; i++) {
        byPriceCounts.value.push(res.data[stepArr[i]].count);
        byPriceIntervals.value.push(
          `${res.data[stepArr[i]].min.toLocaleString()}원~${res.data[
            stepArr[i]
